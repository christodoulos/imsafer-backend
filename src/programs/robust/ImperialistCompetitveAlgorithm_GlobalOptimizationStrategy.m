function [empoptcost, empoptx] = ImperialistCompetitveAlgorithm_GlobalOptimizationStrategy(ProblemParams)
    ...
    function findopt(cost, x)  %Thanasis2021_12_25
    ...
    for Decade = 1:AlgorithmParams.NumOfDecades
        ...
        printmin(Remained);   %Thanasis2021_12_25
        for ii = 1:numel(Empires)
            ...
            findopt(Empires(ii).ColoniesCost, Empires(ii).ColoniesPosition)  %Thanasis2021_12_25
...
function printmin(Remained)
    %Print current minimum at everys step (decade).
    global eplate;   %Thanasis2021_12_25
    fprintf('Current emin: %.15f    Remaining decades: %d\n', eplate.emin, Remained/2);
end
